...
def test():
	...
	test_count = 0
	with torch.no_grad():
		while test_count + batch_size <= test_size:
			input = Variable(data_X[test_count:test_count + batch_size])
			target = Variable(data_Y[test_count:test_count + batch_size])
			test_count += batch_size

			module_stack, output = act_store_model(input)

			logging.info('sample saliency map generation')
			saliency_map = DTD(module_stack, output, 1000, 'TCN')
			...
			heatmap_name = f"{int(test_count / batch_size)}th_sample.png"
			cv2.imwrite(os.path.join(sample_dir, heatmap_name), saliency_heatmap)
			sample_origin = input.cpu().data[0]
			origin_name = f"{int(test_count / batch_size)}th_origin.png"
